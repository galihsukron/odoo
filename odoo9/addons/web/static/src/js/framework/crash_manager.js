...
var CrashManager = core.Class.extend({
    ...
    rpc_error: function(error) {
        ...
        if (error.data.name === "openerp.http.SessionExpiredException" || error.data.name === "werkzeug.exceptions.Forbidden") {
            this.show_warning({type: "Session Expired", data: { message: _t("Your AADC session expired. Please refresh the current web page.") }});
            return;
        }
    ...
    show_warning: function(error) {
        ...
        new Dialog(this, {
            size: 'medium',
            title: "AADC " + (_.str.capitalize(error.type) || _t("Warning")),
            subtitle: error.data.title,
            $content: $('<div>').html(QWeb.render('CrashManager.warning', {error: error}))
        }).open();
    },
    show_error: function(error) { if(error && error.message && typeof error.message === "string" )  error.message = error.message.replace("Odoo", "AADC"); 
        if (!this.active) {
            return;
        }
        new Dialog(this, {
            title: "AADC " + _.str.capitalize(error.type),
...
var RedirectWarningHandler = Dialog.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new Dialog(this, {
            size: 'medium',
            title: "AADC " + (_.str.capitalize(error.type) || "Warning"),
            buttons: [
