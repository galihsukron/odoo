...
#x2nie was here:
INSIDE_PYDEV = 'sitecustomize' in sys.modules
if True and os.name == 'nt': #Windows
    
    #import ntlogging_oe6
...
#    BOLD_SEQ = "\033[1m"
    TEXT_SEQ = "%sm"
    #COLOR_PATTERN = "%s%s%s%s" % (COLOR_SEQ, COLOR_SEQ, TEXT_SEQ, RESET_SEQ)
    #COLOR_PATTERN = "%s%s%%sm%s" % (COLOR_SEQ, COLOR_SEQ, RESET_SEQ)
LEVEL_COLOR_MAPPING = {
...
def init_logger():
    ...
    if tools.config['syslog']:
        ...
        if os.name == 'nt' and not INSIDE_PYDEV:
            #import ntlogging            
            #handler = ntlogging_oe6.NT_StreamHandler(sys.stdout)
    ...
    if INSIDE_PYDEV or isinstance(handler, logging.StreamHandler) and is_a_tty(handler.stream):
        #formatter = ColoredFormatter(format)
        formatter = ColoredFormatter(format, datefmt=datefmt)
    else:
